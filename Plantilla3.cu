...
//Device Code
__global__ void Negativiza(float *Tratada, float *Original, unsigned int nDimX, unsigned int nDimY)
{
	unsigned int i;
...
int main(int argc, char* argv[]) {
   ...
   {
      if ((i % 2) != 0)
         Negativiza<<<BloquesPorGrid, HilosPorBloque>>>(GPUTratada,  GPUOriginal,  nDimX, nDimY);
      else
         Negativiza<<<BloquesPorGrid, HilosPorBloque>>>(GPUOriginal, GPUTratada, nDimX, nDimY);
   }
   printf("acaba");
